...
def make_noise_additive(
    ...
    for (x, y) in coordinates:
        ...
        addition = uniform(range_start, range_end)
        new_image.pixels[y][x] = (
            p[0] + addition if chosen_channel == 0 else p[0],
            p[1] + addition if chosen_channel == 1 else p[1],
            p[2] + addition if chosen_channel == 2 else p[2],
        )
